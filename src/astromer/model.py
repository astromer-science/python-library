...
from astromer.components.decoder  import RegLayer,  NSP_Regressor
from astromer.components.encoder  import Encoder
from astromer.training.losses     import custom_rmse
from astromer.training.metrics    import custom_r2
from astromer.data                import load_numpy, inference_pipeline
...
class ASTROMER(Model):
    ...
    def from_pretrained(self, name):
        if not os.path.isdir('src/astromer/{}'.format(name)):
            download_weights(name, 'src/astromer')
        else:
            self.load_weights('src/astromer/{}'.format(name))
